...
class Wetter:
	...
	def __init__(self, file, verbose, theta = 104.5, waterFrac=0.4, hydroxylFrac = 0.1, MOBondlength = 2, MEnvironment = 5, HOHBondlength = 1, OHBondlength = 1):
		#Input parameters
		self.theta = theta
	...
	def createWater(self, coords, vectors):
		...
		while i < len(coords):
			...
			#Save atoms to be added to pbd file
		...
		pbdAppender.append_atoms(file = self.file, coords = atoms, elements = elements)
	...
	def wet(self):
		...
		#randCoords = random.sample(self.ind, self.waterFrac * len(coords))
		#Check for overlap
